...
export default function LocationMapEmbed() {
  ...
  return (
    <div
      className="relative h-96 w-full overflow-hidden rounded-lg bg-stone-200 shadow-lg"
      aria-busy={!mapReady}
      aria-label="Interactive map of our London location"
    >
      <iframe
        src={MAP_EMBED_SRC}
        width="100%"
      ...
      <div
        className={`absolute inset-0 flex flex-col items-center justify-center gap-3 bg-stone-100 transition-opacity duration-300 ${
          mapReady ? 'pointer-events-none opacity-0' : 'opacity-100'
        }`}
        aria-hidden={mapReady}
      >
        <span
          className="h-10 w-10 animate-spin rounded-full border-[3px] border-[#D2C1B6] border-t-[#234C6A]"
          role="presentation"
        />
